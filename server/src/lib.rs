...
pub enum InitError {
    ...
    DeserializeServerConfig { source: generated_types::ProstError },
...
async fn maybe_initialize_server(shared: &ServerShared) {
    ...
    .map(|server_config| {
        server_config
            ...
            .map(|db| {
                (
                    DatabaseName::new(db.name).expect("serialized db names should be valid"),
                    db.location,
                )
            })
            .collect::<Vec<_>>()
    ...
    let next_state = match maybe_databases {
        Ok(databases) => {
            ...
            for (db_name, _location) in databases {
                state
                    .new_database(
                        shared,
                        DatabaseConfig {
...
mod tests {
    ...
    async fn init_deleted_database() {
        ...
        // create database
        let foo_db = create_simple_database(&server, &foo_db_name)
            .await
            .expect("failed to create database");
        let foo_root_path = foo_db.iox_object_store().unwrap().root_path();
        ...
        let config = server_config(application.object_store(), server_id).await;
        assert_config_contents(&config, &[(&foo_db_name, foo_root_path)]);
